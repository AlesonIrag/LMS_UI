...
import { StudentEventTrackerService } from './student-event-tracker.service';
...
export class StudentAuthService {
  ...
  constructor(
    ...
    private eventTracker: StudentEventTrackerService
  ) {
    ...
    this.eventTracker.cleanupOldEvents();
    this.eventTracker.periodicCleanup();
  }
    ...
    return this.apiService.login({ studentId, password }).pipe(
      ...
      map((response: any) => {
        ...
        if (response && response.success && response.data) {
          ...
          // Track login event
          this.eventTracker.trackLogin(student.studentId, student.fullName, this.getClientIP());
  ...
  logout(): void {
    ...
    // Get current student for event tracking
    ...
    // 6. Track logout event if we have student data
    if (currentStudent) {
      this.eventTracker.trackLogout(currentStudent.studentId, currentStudent.fullName, this.getClientIP());
    }

    // 7. Optionally invalidate token on backend (fire and forget)
    if (token) {
  ...
  validateSession(): Observable<boolean> {
    ...
    if (!student || !token || !loginTimestamp) {
      ...
      // Track logout event due to session invalidation
      if (student) {
        this.eventTracker.trackLogout(student.studentId, student.fullName, this.getClientIP());
      }
      this.logout();
      return of(false);
    }
    ...
    try {
      ...
      if (now - timestamp >= sessionDuration) {
        ...
        // Track logout event due to session expiration
        this.eventTracker.trackLogout(student.studentId, student.fullName, this.getClientIP());
        this.logout();
        return of(false);
      }
      ...
      // Track logout event due to session error
      if (student) {
        this.eventTracker.trackLogout(student.studentId, student.fullName, this.getClientIP());
      }
      this.logout();
    ...
    return this.apiService.validateStudentSession(token).pipe(
      map((response: any) => {
        if (response && response.success && response.data) {
          ...
          // Track logout event due to backend validation failure
          if (student) {
            this.eventTracker.trackLogout(student.studentId, student.fullName, this.getClientIP());
          }
          this.logout();
      ...
      catchError((error) => {
        ...
        // Track logout event due to backend error
        if (student) {
          this.eventTracker.trackLogout(student.studentId, student.fullName, this.getClientIP());
        }
        this.logout();
   ...
   * Note: This is a simplified approach. In production, you might want to get this from the backend.
   */
  private getClientIP(): string | undefined {
    // This is a placeholder. In a real application, you would get this from:
    // 1. A backend API endpoint that returns the client IP
    // 2. A header set by a reverse proxy
